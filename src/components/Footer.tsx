    ...
    <footer className="bg-muted border-t border-border py-12">
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          <div className="md:col-span-1">
            ...
            <div className="flex space-x-4">
              {['Twitter', 'Discord', 'LinkedIn'].map((social, i) => (
                <a key={i} href="#" className="text-gray-600 hover:text-foreground transition-colors">
                  {social}
                </a>
              ))}
            </div>
          ...
          {/* Quick links */}
          <div>
            <h3 className="text-foreground font-medium mb-4">Explore</h3>
            <ul className="space-y-2">
              {['How It Works', 'Universe Eye', 'Dashboard', 'FAQ'].map((link, i) => (
                <li key={i}>
                  <a href={`#${link.toLowerCase().replace(' ', '-')}`} className="text-gray-600 hover:text-foreground transition-colors">
                    {link}
                  </a>
                </li>
              ))}
        ...
        <div className="mt-12 pt-8 border-t border-border flex flex-col md:flex-row justify-between items-center">
          <div className="text-gray-500 text-sm mb-4 md:mb-0">
            © 2025 Quantum AI. All rights reserved.
          </div>
